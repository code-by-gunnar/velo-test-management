  ...
  fastify.get<{
    ...
    async (request, reply) => {
      ...
      const data = await withWorkspace(workspaceId, async (tx) => {
        // 1. Run trend — last 20 completed runs with stats
        const runTrend = await tx`
          SELECT
            tr.id AS run_id,
            tr.name AS run_name,
        ...
        // 2. Fragile cases — top 10 most-failing in last 30 days
        const fragileCases = await tx`
          SELECT
            tc.id AS case_id,
        ...
        // 3. Recent runs — last 10 with stats
